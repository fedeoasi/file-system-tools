...
object CopyFromApplePhotoLibrary {
  def main(args: Array[String]): Unit = {
    ...
    val entries = EntryPersistence.read(metadataFile).filter(_.path.contains("/Masters/"))
    ...
    println(s"There are ${uniqueFiles.size} unique files of total size $totalSize for extension $extension in catalog $metadataFile")
